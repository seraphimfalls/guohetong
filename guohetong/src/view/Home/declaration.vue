  ...
  <div>
    ...
    <div class="width96">
      <div class="width96 font28 margin2 flex-box-between border-bottom2 textLeft">
        <span style="width:35%">合伙人</span>
        <span class="corloraeaeae">{{order_price}}元（财富级）</span>
      </div>
      <div class="width96 font28 margin2 flex-box-between border-bottom2 textLeft">
        <span style="width:35%">使用金种子</span>
        <span class="corloraeaeae">{{order_price1}}个</span>
      </div>
      <div class="width96 font28 margin2 flex-box-between border-bottom2 textLeft">
        <span style="width:35%">使用备用金</span>
        <van-field type="number" v-model="num" @blur="changeNum" :placeholder="`请输入使用个数（最多可用 ${order_score}）个`" />
      </div>
      <div class="width96 font28 margin2 flex-box-between border-bottom2 textLeft">
        <span style="width:35%">报单对象</span>
        <van-field type="number" v-model="mobile" placeholder="请输入报单手机号" />
      </div>
      <div class="width96 font28 margin2 flex-box-between border-bottom2 textLeft">
    ...
    <van-cell class="textLeft font28" title="报单说明：" size="large" :label="`报单${order_price}最多可用${order_score}个备用金`" />
     <div class="btn-box">
       <van-button type="info" @click="confirm">确认报单</van-button>
    </div>
  </div>
</template>
<script>
...
async created(){
  ...
  this.order_price1 = res.data.order_price;
  ...
  methods: {
    changeNum(){
      if(this.num){
         this.order_price1 = parseFloat(this.order_price)-parseFloat(this.num);
      }else if(this.num==''){
        this.num=0
        this.order_price1 =this.order_price
      }
    },
    async confirm(){
      const res = await assetsApi.applyOrder(this.order_price1,this.num,this.mobile,this.password);
      if(res.code==1){
