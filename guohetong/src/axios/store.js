...
//商城banner
export async function shoppingBanner() {
  const formData = createFormData({
  });
  return await http.post("/api/Page/shoppingBanner", formData);
}
//热卖商品列表
export async function getHotGoodsList(page,limit) {
  const formData = createFormData({
    page,
    limit
  });
...
//商品列表
export async function getGoodsList(page,limit) {
  const formData = createFormData({
    page,
    limit
  });
  return await http.post("/api/User/getGoodsList", formData);
}
//获取商品详情
export async function getGoodsDetail(id) {
  ...
  return await http.post("/api/User/getGoodsDetail", formData);
}
//获取商品分类
...
//活动商品下单
export async function createSpecialGoodsOrder(address_id,id,goods_num) {
  const formData = createFormData({
    address_id,
    id,
    goods_num
  });
  return await http.post("/api/Shoppingorder/createSpecialGoodsOrder", formData);
}
//确认订单页信息详情
